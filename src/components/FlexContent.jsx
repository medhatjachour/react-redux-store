...
const FlexContent = ({ isHighlight, endpoint : { title, heading, text ,btn,url, img }}) => {
  return (
    <>
        <div className={`flex items-center justify-between lg:flex-col lg:justify-center hero-main-container ${isHighlight?'flex-row-reverse':'flex-row'}`}>
          ...
          <img
            src={img}
            alt={`img/${heading}`}
            className={`w-auto object-fill transitions-theme ${isHighlight ? 'h-60 lg:h-56 md:h-52 sm:h-44 xsm:h-36 rotate-6 hover:-rotate-12' : 'h-72 lg:h-64 md:h-60 sm:h-48 xsm:h-40 rotate-[19deg] hover:rotate-12'}`}
          />
        </div>
        </div>
    </>
